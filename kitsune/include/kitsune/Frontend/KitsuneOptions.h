...
private:
  ...
  /// Should the LLVM Module be printed to stderr just before running the
  /// optimization passes. Tapir currently requires at least -O1. This makes it
  /// difficult to write tests that check that the frontend generates the
  /// expected LLVM IR. If this restriction is ever removed, this option will no
  /// longer be necessary since we could just pass -O0 to examine the IR
  /// generated by the frontend.
  unsigned printBeforeFirst : 1;
...
public:
  ...
  void setPrintBeforeFirst(bool printBeforeFirst = true) {
    this->printBeforeFirst = printBeforeFirst;
  }
  /// @}
  ...
  bool getPrintBeforeFirst() const { return printBeforeFirst; }
